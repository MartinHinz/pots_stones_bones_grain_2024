...
if (download) {
  ...
  # Download the pollen data for Switzerland from Neotoma
  ch_sites <- neotoma2::get_sites(gpid = "Switzerland",
                                  datasettype = "pollen",
                                  minage = 2000,
                                  maxage = 8000,
                                  all_data = TRUE)
  ch_datasets <- neotoma2::get_datasets(ch_sites, all_data = TRUE)
  ch_dl <- ch_datasets %>% get_downloads(all_data = TRUE)

  # Store the data for further use
  saveRDS(ch_dl, file = here("_data", "ch_pollen.RDS"))

} else {

  # if download is 'false', read the stored data from the data folder
  ch_dl <- readRDS(ch_dl, file = here("_data", "ch_pollen.RDS"))
}

# Filter the pollen data
ch_all_samples <- samples(ch_dl)  %>%
...
if (download) {
  ch_radiocarbon <- chron_data(country = "Switzerland")

  # Store the data for further use
  saveRDS(ch_radiocarbon, file = "../_data/ch_radiocarbon.RDS")

} else {

  # if download is 'false', read the stored data from the data folder
  ch_radiocarbon <- readRDS(ch_radiocarbon,
                            file = here("_data", "ch_radiocarbon.RDS"))
}

### Cereals

# filter for cereal data
ch_cereals <- ch_radiocarbon %>% filter(str_detect(material, "grain"))

# remove duplicates
duplicates <- duplicated(ch_cereals[c("labnr", "bp", "std")])
ch_cereals <- ch_cereals[!(duplicates), ] # Remove duplicates

# Calibrate the data using rcarbon
ch_cereals_cal <- rcarbon::calibrate(x = ch_cereals$bp, errors = ch_cereals$std)

# Calculate the SPD for cereals
ch_cereals_spd <- spd(ch_cereals_cal,
                      timeRange = c(7000, 3000),
                      bins = ch_cereals$site)

# store the plot for later use
p2 <- ggplot(ch_cereals_spd$grid) +
  geom_line(aes(x = 1950 - calBP, y = PrDens)) +
  theme_bw() +
  ggtitle(paste("SPD of cereal samples, n=",
                nrow(ch_cereals),
                "from",
                length(unique(ch_cereals$site)),
                "sites"))
...
ch_all_radiocarbon <- ch_radiocarbon

# remove duplicates
duplicates <- duplicated(ch_all_radiocarbon[c("labnr", "bp", "std")])
ch_all_radiocarbon <- ch_all_radiocarbon[!(duplicates), ] # Remove duplicates

# exclude all data outside of calibration range
ch_all_radiocarbon <- ch_all_radiocarbon %>% filter(bp > 500 & bp < 50000)

# calibrate using rcarbon
ch_radiocarbon_cal <- rcarbon::calibrate(x = ch_all_radiocarbon$bp,
                                         errors = ch_all_radiocarbon$std)
...
if (download) {
  expnull <- modelTest(ch_radiocarbon_cal,
                       errors = ch_all_radiocarbon$std,
                       bins = ch_all_radiocarbon$site,
                       nsim = nsim,
                       timeRange = c(7000, 3000),
                       model = "exponential",
                       runm = 100)
  ...
  # Store the data for further use
  saveRDS(expnull, file = here("_data", "ch_expnull.RDS"))

} else {
  # if download is 'false', read the stored data from the data folder
  expnull <- readRDS(expnull, file = here("_data", "ch_expnull.RDS"))
}
...
p3 <- ggplot(expnull$result) + geom_line(aes(x = 1950 - calBP, y = PrDens)) +
  ...
  ggtitle(
    paste("SPD of all radiocarbon data, n=",
          nrow(ch_all_radiocarbon),
          "from",
          length(unique(ch_all_radiocarbon$site)),
          "sites")
  )
...
processes <- rbind(
  ...
  data.frame(group = "Megaliths",
             label = "Chamblandes tombs (submegalithic)",
             begin = -4500,
             end = -3750),
  data.frame(group = "Megaliths",
             label = "Dolmen (use)",
